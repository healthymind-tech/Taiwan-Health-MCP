...
| **Settings** | DB-backed 設定（MinIO / TFDA / worker 調校）與 **LLM Profiles**（嵌入 / OCR / 分析 LLM 端點），可線上測試與熱套用。 |
...
Bootstrap 變數（DB / Redis / MCP transport / `ADMIN_*` 認證）只存在於 `.env`。
...
僅能在 Settings 頁籤設定。可用 Settings → Export / Import 在不同環境間搬移一份可用的設定。
