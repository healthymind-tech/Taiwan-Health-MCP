...
| `token_strategy` | string | No | Override the server's default token strategy (`fresh` / `cached`) |
...
- The path is composed from `operation` / `resource_type` / `resource_id`; the caller does **not** pass a URL directly.
