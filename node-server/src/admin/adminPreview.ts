...
async function previewGuideline(params: Params): Promise<Row> {
  const idIn = (params.id_ ?? null) as number | null;
  const qIn = (params.q ?? null) as string | null;

  const total = n(
    (await query("SELECT COUNT(*) AS c FROM guideline.disease_guidelines")).rows[0]?.c,
  );
  if (total === 0) {
    return {
      type: "empty",
  ...
  if (qIn && qIn.trim()) {
    ...
    return {
      ...
      results: rows.map((r) => ({
        id: r.id,
        icd_code: r.icd_code,
        disease_name_zh: r.disease_name_zh || "",
  ...
  if (idIn !== null) {
    const g = (await query("SELECT * FROM guideline.disease_guidelines WHERE id = $1", [idIn]))
      .rows[0];
    if (!g) return { type: "error", message: `Guideline id=${idIn} not found` };
    ...
    return {
      ...
      guideline: {
        id: g.id,
        icd_code: g.icd_code,
        disease_name_zh: g.disease_name_zh || "",
        disease_name_en: g.disease_name_en || "",
        guideline_title: g.guideline_title || "",
        guideline_source: g.guideline_source || "",
        publication_year: g.publication_year,
        guideline_summary: g.guideline_summary || "",
      },
      diagnostic_recommendations: diag,
