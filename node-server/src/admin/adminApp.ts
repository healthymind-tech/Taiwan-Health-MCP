...
import {
  registerGuidelineSourceDocument,
  registerGuidelineSourceDocumentFromUrl,
  getGuidelineAdminStatus,
  getGuidelineAssetContent,
  getGuidelineAnalysisAssetContent,
  getGuidelinePipelineStatus,
  icdCodeExists,
  GuidelineValueError,
  type GuidelineAssetKind,
} from "./adminGuideline.js";
import {
  approveReview as approveGuidelineReview,
  getReviewDetail as getGuidelineReviewDetail,
  listPendingReviews as listPendingGuidelineReviews,
  rejectReview as rejectGuidelineReview,
  ReviewValueError,
...
export async function adminHandler(req: Request, res: Response, next: NextFunction): Promise<void> {
  ...
  try {
    ...
    // GET /admin/api/guideline/status
    if (method === "GET" && path === "/admin/api/guideline/status") {
      try {
        const page = parseIntDefault(req.query.page, 1, (v) => Math.max(1, v));
        const perPage = parseIntDefault(req.query.per_page, 50, (v) =>
          Math.max(1, Math.min(200, v)),
        ...
        sendJson(
          res,
          200,
          await getGuidelineAdminStatus({
            page,
            perPage,
            icdCode: String(req.query.icd_code ?? "").trim() || undefined,
            stage: String(req.query.stage ?? "").trim() || undefined,
          }),
        );
      } catch (exc) {
        sendJson(res, 500, {
          error: "Failed to load guideline admin status",
          detail: String((exc as Error).message),
        });
      }
      return;
    ...
    // GET /admin/api/guideline/pipeline-status
    if (method === "GET" && path === "/admin/api/guideline/pipeline-status") {
      try {
        sendJson(res, 200, await getGuidelinePipelineStatus());
      } catch (exc) {
        sendJson(res, 500, {
          error: "Failed to load guideline pipeline status",
          detail: String((exc as Error).message),
        });
      }
      return;
    ...
    // POST /admin/api/guideline/documents — register an already-uploaded PDF
    // (uploaded via the generic POST /admin/api/uploads with
    // module_key=guideline&source_role=guideline_pdf) as a
    // guideline.source_documents row the analysis pipeline picks up. No ICD
    // code is declared here — the Analysis LM determines it from the document.
    if (method === "POST" && path === "/admin/api/guideline/documents") {
      const body = (req.body ?? {}) as Record<string, unknown>;
      const uploadedFileId = String(body.uploaded_file_id ?? "").trim();
      if (!uploadedFileId) {
        sendJson(res, 400, { error: "uploaded_file_id is required" });
      ...
      try {
        const document = await registerGuidelineSourceDocument({
          uploadedFileId,
          uploadedBy: adminUsername,
        });
        sendJson(res, 201, { document });
      } catch (exc) {
        if (exc instanceof GuidelineValueError) sendJson(res, 400, { error: exc.message });
        else {
          sendJson(res, 500, {
            error: "Failed to create guideline source document",
            detail: String((exc as Error).message),
          });
        }
      }
    ...
    // POST /admin/api/guideline/documents/from-url — same end result as
    // POST /admin/api/guideline/documents, but the PDF is fetched server-side
    // from an admin-supplied HTTPS URL (SSRF-guarded, see safeUrlFetch.ts)
    // instead of uploaded from the browser.
    if (method === "POST" && path === "/admin/api/guideline/documents/from-url") {
      const body = (req.body ?? {}) as Record<string, unknown>;
      ...
      try {
        const document = await registerGuidelineSourceDocumentFromUrl({ url, uploadedBy: adminUsername });
        sendJson(res, 201, { document });
      } catch (exc) {
        if (exc instanceof GuidelineValueError || exc instanceof SourceValueError) {
          sendJson(res, 400, { error: exc.message });
        } else if (exc instanceof SourceRuntimeError) {
          sendJson(res, 503, { error: exc.message });
        } else {
          sendJson(res, 500, {
            error: "Failed to fetch guideline PDF from URL",
            detail: String((exc as Error).message),
          });
        }
      }
    ...
    // GET /admin/api/guideline/icd-exists?code=E11 — live-edit validation backing
    // the review UI's Approve-button gate; the authoritative check still happens
    // server-side in approveReview.
    if (method === "GET" && path === "/admin/api/guideline/icd-exists") {
      const code = String(req.query.code ?? "").trim();
      try {
        sendJson(res, 200, { code, exists: code ? await icdCodeExists(code) : false });
      } catch (exc) {
        sendJson(res, 500, { error: "Failed to check ICD code", detail: String((exc as Error).message) });
      }
      return;
    }

    // GET /admin/api/guideline/asset-content?kind=pdf|ocr|analysis&document_id=...&analysis_id=...
    // Same-origin proxy that streams one artefact's bytes from MinIO, mirroring
    // /admin/api/drug/asset-content. pdf/ocr are document-scoped (one per PDF,
    // shared across every extraction); analysis is extraction-scoped (analysis_id).
    if (method === "GET" && path === "/admin/api/guideline/asset-content") {
      const kind = String(req.query.kind ?? "").trim() as GuidelineAssetKind | "analysis";
      if (!["pdf", "ocr", "analysis"].includes(kind)) {
        sendJson(res, 400, { error: "kind must be one of: pdf, ocr, analysis" });
        return;
      }
      const documentId = String(req.query.document_id ?? "").trim();
      const analysisId = String(req.query.analysis_id ?? "").trim();
      if (kind === "analysis" ? !analysisId : !documentId) {
        sendJson(res, 400, {
          error: kind === "analysis" ? "analysis_id is required" : "document_id is required",
        });
        return;
      }
      try {
        const asset =
          kind === "analysis"
            ? await getGuidelineAnalysisAssetContent(analysisId)
            : await getGuidelineAssetContent(documentId, kind);
        if (asset === null) {
          sendJson(res, 404, { error: "Asset not found or has no stored content" });
          return;
        }
        ...
        const range = isPdf ? parseByteRange(req.headers.range, asset.data.length) : undefined;
        if (range === null) {
          res.writeHead(416, { "Content-Range": `bytes */${asset.data.length}`, "Accept-Ranges": "bytes" });
          res.end();
          return;
        }
        const body = range ? asset.data.subarray(range.start, range.end + 1) : asset.data;
        ...
        sendJson(res, 500, {
          error: "Failed to load guideline asset content",
          detail: String((exc as Error).message),
        });
      }
      return;
    ...
    if (method === "GET" && path === "/admin/api/guideline/review/pending") {
      try {
        const page = parseIntDefault(req.query.page, 1, (v) => Math.max(1, v));
        const perPage = parseIntDefault(req.query.per_page, 50, (v) =>
          Math.max(1, Math.min(200, v)),
        );
        sendJson(res, 200, await listPendingGuidelineReviews({ page, perPage }));
      } catch (exc) {
        sendJson(res, 500, {
          error: "Failed to list pending guideline reviews",
          detail: String((exc as Error).message),
        });
      }
      return;
    ...
    // GET /admin/api/guideline/review/:analysisId — one disease's extraction
    // from within a (possibly multi-disease) source document.
    if (method === "GET" && path.startsWith("/admin/api/guideline/review/")) {
      const analysisId = path.slice("/admin/api/guideline/review/".length).split("/")[0];
      try {
        sendJson(res, 200, await getGuidelineReviewDetail(analysisId));
      } catch (exc) {
        if (exc instanceof ReviewValueError) sendJson(res, 404, { error: exc.message });
        else {
          sendJson(res, 500, {
            error: "Failed to load guideline review detail",
            detail: String((exc as Error).message),
          });
        }
      }
    ...
    // POST /admin/api/guideline/review/:analysisId/approve
    if (
      method === "POST" &&
      path.startsWith("/admin/api/guideline/review/") &&
      path.endsWith("/approve")
    ) {
      const analysisId = path.slice(
        "/admin/api/guideline/review/".length,
        path.length - "/approve".length,
      );
      const body = (req.body ?? {}) as Record<string, unknown>;
      try {
        const result = await approveGuidelineReview(analysisId, {
          edits: body.edits,
          reviewedBy: adminUsername,
          notes: body.notes ? String(body.notes) : undefined,
        });
        sendJson(res, 200, { guideline_id: result.guidelineId });
      } catch (exc) {
        if (exc instanceof ReviewValueError) sendJson(res, 400, { error: exc.message });
        else {
          sendJson(res, 500, {
            error: "Failed to approve guideline review",
            detail: String((exc as Error).message),
          });
        }
      }
    ...
    // POST /admin/api/guideline/review/:analysisId/reject
    if (
      method === "POST" &&
      path.startsWith("/admin/api/guideline/review/") &&
      ...
      const body = (req.body ?? {}) as Record<string, unknown>;
      try {
        await rejectGuidelineReview(analysisId, {
          reviewedBy: adminUsername,
        ...
        if (exc instanceof ReviewValueError) sendJson(res, 400, { error: exc.message });
          ...
          sendJson(res, 500, {
            error: "Failed to reject guideline review",
            detail: String((exc as Error).message),
          });
        }
      }
